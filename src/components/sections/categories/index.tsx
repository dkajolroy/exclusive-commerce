...
import { getSubcategoriesSSG } from "@/hooks/getSubcategories";
import CategorySlider from "./categorySlider";

async function Categories() {
  const url = `${process.env.NEXTAUTH_URL}/api/subcategories`;
  const { subcategory } = await getSubcategoriesSSG(url);
  ...
  return (
    <div className="container py-10 border-b">
      ...
      <div className="py-5">
        {subcategory.length && <CategorySlider categories={subcategory} />}
      </div>
    </div>
  );
}
